    ...
    public sealed class 客户网络
    {
        ...
        private void 处理已收封包()
        {
            while (!this.接收列表.IsEmpty)
            {
                ...
                if (this.接收列表.TryDequeue(out GamePacket packet))
                {
                    if (Config.DebugPackets)
                    {
                        MainForm.AddSystemLog($"[Packet-Rcv] {packet}");
                    }
                    if (!GamePacket.封包处理方法表.TryGetValue(packet.封包类型, out MethodInfo methodInfo))
                    {
                        this.尝试断开连接(new Exception("No packet handling found, disconnect. Packet type: " + packet.封包类型.FullName));
                        return;
                    }
