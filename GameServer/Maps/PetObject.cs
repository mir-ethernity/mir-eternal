...
namespace GameServer.Maps
{
	...
	public sealed class PetObject : MapObject
	{
		...
		// Token: 0x0600080A RID: 2058 RVA: 0x0003F7C4 File Offset: 0x0003D9C4
		public PetObject(PlayerObject 宠物主人, PetData 对象数据)
		{
			...
			this.攻击时间 = MainProcess.当前时间.AddSeconds(1.0);
			this.漫游时间 = MainProcess.当前时间.AddMilliseconds((double)(MainProcess.随机数.Next(5000) + this.漫游间隔));
		...
		// Token: 0x0600080B RID: 2059 RVA: 0x0003FAD8 File Offset: 0x0003DCD8
		...
		// Token: 0x0600080C RID: 2060 RVA: 0x0003FDF8 File Offset: 0x0003DFF8
		public PetObject(PlayerObject 宠物主人, MonsterObject 诱惑怪物, byte 初始等级, bool 绑定武器, int 宠物时长)
		{
			...
			this.宠物主人 = 宠物主人;
			this.对象模板 = 诱惑怪物.对象模板;
			this.PetData = new PetData(诱惑怪物.对象名字, 初始等级, 7, 绑定武器, MainProcess.当前时间.AddMinutes((double)宠物时长));
			this.当前坐标 = 诱惑怪物.当前坐标;
			this.当前地图 = 诱惑怪物.当前地图;
			this.当前方向 = 诱惑怪物.当前方向;
			this.属性加成[this] = this.基础属性;
			...
			base.恢复时间 = MainProcess.当前时间.AddSeconds(5.0);
			this.攻击时间 = MainProcess.当前时间.AddSeconds(1.0);
			this.忙碌时间 = MainProcess.当前时间.AddSeconds(1.0);
			this.漫游时间 = MainProcess.当前时间.AddMilliseconds((double)this.漫游间隔);
			this.HateObject = new HateObject();
			string text = this.对象模板.普通攻击技能;
			if (text != null && text.Length > 0)
			{
		...
		// Token: 0x0600080D RID: 2061 RVA: 0x000400D0 File Offset: 0x0003E2D0
		public PetObject(PlayerObject 宠物主人, PetObject 诱惑宠物, byte 初始等级, bool 绑定武器, int 宠物时长)
		{
			...
			this.宠物主人 = 宠物主人;
			this.对象模板 = 诱惑宠物.对象模板;
			this.PetData = new PetData(诱惑宠物.对象名字, 初始等级, 7, 绑定武器, MainProcess.当前时间.AddMinutes((double)宠物时长));
			this.当前坐标 = 诱惑宠物.当前坐标;
			this.当前地图 = 诱惑宠物.当前地图;
			this.当前方向 = 诱惑宠物.当前方向;
			this.属性加成[this] = this.基础属性;
		...
		// Token: 0x0600080E RID: 2062 RVA: 0x000403A8 File Offset: 0x0003E5A8
		public override void 处理对象数据()
		{
			...
			else if (this.叛变时间 != default(DateTime) && MainProcess.当前时间 > this.叛变时间)
			{
			...
			else
			{
				...
				if (MainProcess.当前时间 > base.治疗时间 && base.治疗次数 > 0)
				{
					int 治疗次数 = base.治疗次数;
					base.治疗次数 = 治疗次数 - 1;
				...
				if (this.进入战斗技能 != null && !base.战斗姿态 && this.HateObject.仇恨列表.Count != 0)
				{
					...
					base.战斗姿态 = true;
					base.脱战时间 = MainProcess.当前时间.AddSeconds(10.0);
				}
				else if (this.退出战斗技能 != null && base.战斗姿态 && this.HateObject.仇恨列表.Count == 0 && MainProcess.当前时间 > base.脱战时间)
				{
				...
				else if (this.宠物主人.PetMode == PetMode.攻击 && MainProcess.当前时间 > this.忙碌时间 && MainProcess.当前时间 > this.硬直时间)
				{
		...
		// Token: 0x0600080F RID: 2063 RVA: 0x000406D0 File Offset: 0x0003E8D0
		public override void 自身死亡处理(MapObject 对象, bool 技能击杀)
		{
			...
			base.自身死亡处理(对象, 技能击杀);
			this.消失时间 = MainProcess.当前时间.AddMilliseconds((double)this.尸体保留);
			this.清空宠物仇恨();
		...
		// Token: 0x06000810 RID: 2064 RVA: 0x00040840 File Offset: 0x0003EA40
		public void 宠物智能跟随()
		{
			if (!this.能否走动())
			{
				...
				if (base.网格距离(this.宠物主人) > 2 || base.网格距离(point) > 2)
				{
					...
					for (int i = 0; i < 8; i++)
					{
						...
						if (this.当前地图.能否通行(point2))
						{
							...
							this.行走时间 = MainProcess.当前时间.AddMilliseconds((double)(this.行走耗时 + this.移动间隔));
						...
						GameDirection = ComputingClass.旋转方向(GameDirection, (MainProcess.随机数.Next(2) == 0) ? -1 : 1);
					}
					return;
				}
				if (MainProcess.当前时间 > this.漫游时间)
				{
					this.漫游时间 = MainProcess.当前时间.AddMilliseconds((double)(this.漫游间隔 + MainProcess.随机数.Next(5000)));
		...
		// Token: 0x06000811 RID: 2065 RVA: 0x00040A64 File Offset: 0x0003EC64
		public void 宠物智能攻击()
		{
			...
			游戏技能 游戏技能;
			if (this.概率触发技能 != null && (!this.冷却记录.ContainsKey((int)this.普通攻击技能.自身技能编号 | 16777216) || MainProcess.当前时间 > this.冷却记录[(int)this.普通攻击技能.自身技能编号 | 16777216]) && ComputingClass.计算概率(this.概率触发技能.计算幸运概率 ? ComputingClass.计算幸运(this[GameObjectProperties.幸运等级]) : this.概率触发技能.计算触发概率))
			{
				游戏技能 = this.概率触发技能;
			}
			else
			{
				if (this.普通攻击技能 == null || (this.冷却记录.ContainsKey((int)this.普通攻击技能.自身技能编号 | 16777216) && !(MainProcess.当前时间 > this.冷却记录[(int)this.普通攻击技能.自身技能编号 | 16777216])))
				{
					return;
				}
				游戏技能 = this.普通攻击技能;
			...
			else
			{
				if (MainProcess.当前时间 > this.攻击时间)
				{
					...
					this.攻击时间 = MainProcess.当前时间.AddMilliseconds((double)(ComputingClass.数值限制(0, 10 - this[GameObjectProperties.攻击速度], 10) * 500));
					return;
				}
				if (this.能否转动())
				{
		...
		// Token: 0x06000812 RID: 2066 RVA: 0x00040D34 File Offset: 0x0003EF34
		public void 宠物经验增加()
		{
		...
		// Token: 0x06000813 RID: 2067 RVA: 0x00040DE8 File Offset: 0x0003EFE8
		public void 宠物召回处理()
		{
		...
		// Token: 0x06000814 RID: 2068 RVA: 0x00006A9C File Offset: 0x00004C9C
		public void 宠物沉睡处理()
		{
			this.技能任务.Clear();
			this.Buff列表.Clear();
		...
		// Token: 0x06000815 RID: 2069 RVA: 0x00040E70 File Offset: 0x0003F070
		public bool 更新HateObject()
		{
			...
			else if (base.网格距离(this.HateObject.当前目标) > this.仇恨范围 && MainProcess.当前时间 > this.HateObject.仇恨列表[this.HateObject.当前目标].仇恨时间)
			{
				this.HateObject.移除仇恨(this.HateObject.当前目标);
			}
			else if (base.网格距离(this.HateObject.当前目标) <= this.仇恨范围)
			{
				this.HateObject.仇恨列表[this.HateObject.当前目标].仇恨时间 = MainProcess.当前时间.AddMilliseconds((double)this.仇恨时长);
			}
			if (this.HateObject.切换时间 < MainProcess.当前时间 && this.HateObject.切换仇恨(this))
			{
				this.HateObject.切换时间 = MainProcess.当前时间.AddMilliseconds((double)this.切换间隔);
			}
			return this.HateObject.当前目标 != null || this.更新HateObject();
		}

		// Token: 0x06000816 RID: 2070 RVA: 0x00006AC1 File Offset: 0x00004CC1
		public void 清空宠物仇恨()
		{
