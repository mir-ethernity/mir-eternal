...
namespace GameServer.Maps
{
    public sealed class PlayerObject : MapObject
    {
        ...
        public void 玩家升级处理()
        {
            ...
            if (this.CurrentRank >= 36 && this.所属师门 != null && this.所属师门.师父编号 != this.MapId)
            {
                this.提交出师申请();
            }
        }
        ...
        {
            if (经验增加 > 0 && (this.CurrentRank < Config.游戏OpenLevelCommand || this.CurrentExp < this.所需经验))
            {
                int num = 经验增加;
                int num2 = 0;
                if (怪物 != null)
                ...
                if (num3 > 0)
                {
                    if ((this.CurrentExp += num3) >= this.所需经验 && this.CurrentRank < Config.游戏OpenLevelCommand)
                    {
        ...
        public void SkillGainExp(ushort skillId)
        {
            ...
            {
                DataMonitor<ushort> SkillExp2 = skill.SkillExp;
                SkillExp2.V -= skill.升级经验;
                DataMonitor<byte> SkillLevel = skill.SkillLevel;
                SkillLevel.V += 1;
        ...
        public void 玩家穿卸装备(EquipmentWearingParts itemType, EquipmentData 原有装备, EquipmentData 现有装备)
        {
            ...
            if (原有装备 != null)
            {
                if (原有装备.物品类型 == ItemType.武器)
                {
                    foreach (BuffData BuffData in this.Buff列表.Values.ToList<BuffData>())
                    {
                        if (BuffData.BoundWeapons && (BuffData.Buff来源 == null || BuffData.Buff来源.MapId == this.MapId))
                        {
                            base.删除Buff时处理(BuffData.Id.V);
                        }
                    }
        ...
        public void 玩家进入场景()
        {
            ...
            if (网络连接6 != null)
            {
                网络连接6.发送封包(new 同步NumberDollars
                {
        ...
        public void 请求对象外观(int 对象编号, int 状态编号)
        {
            ...
            else
            {
                ...
                if (PlayerObject != null)
                {
                    ...
                    GuildData Affiliation = PlayerObject.Guild;
                    SyncPlayerAppearancePacket.行会编号 = ((Affiliation != null) ? Affiliation.数据索引.V : 0);
