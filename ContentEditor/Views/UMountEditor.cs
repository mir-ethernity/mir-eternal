...
{
    public partial class UMountEditor : UBaseEditor
    {
        public override string AttachedTabName => "TabMounts";
        ...
        {
            InitializeComponent();
            DataGridMounts.AutoGenerateColumns = false;
            DataGridMounts.DataError += DataGridMounts_DataError;
            DataGridMounts.CellDoubleClick += DataGridMounts_CellDoubleClick;
        }

        private void DataGridMounts_CellDoubleClick(object? sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0)
                return;

            var row = DataGridMounts.Rows[e.RowIndex];
        ...
        private void DataGridMounts_DataError(object? sender, DataGridViewDataErrorEventArgs e)
        {
        ...
        public override void ReloadDatabase(IDatabaseManager database)
        {
            ...
            DataGridMounts.DataSource = database.Item.DataSource;
            DataGridMounts.Refresh();
        }
