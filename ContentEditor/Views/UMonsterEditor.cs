...
{
    public partial class UMonsterEditor : UBaseEditor
    {
        public override string AttachedTabName => "TabMonsters";

        public IDatabaseManager Database { get; private set; }

        public UMonsterEditor()
        {
            InitializeComponent();
            DataGridMonsters.AutoGenerateColumns = false;
            DataGridMonsters.DataError += DataGridMonsters_DataError;
            DataGridMonsters.CellDoubleClick += DataGridMonsters_CellDoubleClick;
        }

        private void DataGridMonsters_CellDoubleClick(object? sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0)
                return;

            var row = DataGridMonsters.Rows[e.RowIndex];
        ...
        private void DataGridMonsters_DataError(object? sender, DataGridViewDataErrorEventArgs e)
        {
        ...
        public override void ReloadDatabase(IDatabaseManager database)
        {
            ...
            DataGridMonsters.DataSource = database.Item.DataSource;
            DataGridMonsters.Refresh();
        }
