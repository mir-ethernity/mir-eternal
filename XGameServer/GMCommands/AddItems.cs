        ...
        public override void Execute()
        {
            ...
            if (GameDataGateway.CharacterDataTable.Keyword.TryGetValue(this.CharName, out GameData))
            {
                ...
                if (CharacterData != null)
                {
                    ...
                    {
                        SEnvir.AddCommandLog("<= @" + base.GetType().Name + " Command execution failed, item does not exist");
                        return;
                    }
                    if (CharacterData.Backpack.Count >= (int)CharacterData.BackpackSize.V)
                    {
                        SEnvir.AddCommandLog("<= @" + base.GetType().Name + " Command execution failed, character's bag is full");
                        return;
                    }
                    if (游戏物品.MaxDura == 0)
                    {
                        SEnvir.AddCommandLog("<= @" + base.GetType().Name + " Command execution failed, cannot AddItemsCommand");
                        return;
                    }
                    ...
                    if (CharacterData.TryGetFreeSpaceAtInventory(out byte b))
                    {
                        ...
                        SEnvir.AddCommandLog("<= @" + base.GetType().Name + " The command has been executed and the item has been added to the character's bag");
                        return;
                    }
                    //goto IL_F4;
                }
