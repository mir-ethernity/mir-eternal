		...
		public override void Execute()
		{
			...
			if (GameDataGateway.CharacterDataTable.Keyword.TryGetValue(this.CurrentCharacterName, out GameData))
			{
				...
				if (CharacterData != null)
				{
					...
					{
						SEnvir.AddCommandLog("<= @" + base.GetType().Name + " Command execution failed, account must be taken offline");
						return;
					}
					if (Encoding.UTF8.GetBytes(this.NewCharacterName).Length > 24)
					{
						SEnvir.AddCommandLog("<= @" + base.GetType().Name + " Command execution failed, character name too long");
						return;
					}
					if (GameDataGateway.CharacterDataTable.Keyword.ContainsKey(this.NewCharacterName))
					{
						SEnvir.AddCommandLog("<= @" + base.GetType().Name + " Command execution failed, name already registered");
						return;
					}
					...
					SEnvir.AddCommandLog(string.Format("<= @{0} command has been executed, with the current name of the character: {1}", base.GetType().Name, CharacterData));
					return;
				}
			}
