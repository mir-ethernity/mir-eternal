	...
	public sealed class BanNIC : GMCommand
	{
		...
		public override void Execute()
		{
			if (Regex.IsMatch(this.MacAddress, "^([0-9a-fA-F]{2}(?:[:-]?[0-9a-fA-F]{2}){5})$"))
			{
				...
				SEnvir.AddCommandLog(string.Format("<= @{0} command executed, blocking expiry time: {1}", base.GetType().Name, DateTime.Now.AddDays((double)this.封禁天数)));
				return;
			}
			SEnvir.AddCommandLog("<= @" + base.GetType().Name + " Command execution failed, address format error");
		}
