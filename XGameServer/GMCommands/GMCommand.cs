...
namespace GameServer
{
    ...
    public abstract class GMCommand
    {
        ...
        public static bool 解析命令(string 文本, out GMCommand cmd)
        {
            ...
            {
                SEnvir.AddCommandLog("<= @" + array[0] + " is not a valid GM command, use @View");
                cmd = null;
                return false;
            }
            ...
            {
                SEnvir.AddCommandLog("<= Parameter length error, please see format: " + 命令格式[array[0]]);
                cmd = null;
                return false;
            }
            ...
            for (int i = 0; i < fields.Length; i++)
            {
                ...
                {
                    SEnvir.AddCommandLog($"<= Parameter conversion error. The string cannot be converted to '{array[i + 1]}' Convert to parameters '{fields[i].Name}' Data type required");
                    cmd = null;
                    return false;
                }
            }
