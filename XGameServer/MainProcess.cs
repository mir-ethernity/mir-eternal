        ...
        private static void MainLoop()
        {
            ...
            while (true)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    var pathLogError = Path.Combine("Log", "Error");
                    if (!Directory.Exists(pathLogError))
                        Directory.CreateDirectory(pathLogError);
                    File.WriteAllText(Path.Combine(pathLogError, string.Format("{0:yyyy-MM-dd--HH-mm-ss}.txt", DateTime.Now)), "Error message:\r\n" + ex.Message + "\r\nStack information:\r\n" + ex.StackTrace);
