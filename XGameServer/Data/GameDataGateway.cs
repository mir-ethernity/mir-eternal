...
namespace GameServer.Data
{
    ...
    public static class GameDataGateway
    {
        ...
        public static string UserFolder
        {
            get
            {
                return Path.Combine(Config.GameDataPath, "User");
            }
        }
        ...
        public static string UserPath
        {
            get
            {
                return Path.Combine(Config.GameDataPath, "User", "Data.db");
            }
        }
        ...
        public static string UserTempPath
        {
            get
            {
                return Path.Combine(Config.GameDataPath, "User", "Temp.db");
            }
        }
        ...
        public static string UserBackupPath
        {
            get
            {
                return Path.Combine(Config.BackupFolder, string.Format("User-{0:yyyy-MM-dd-HH-mm-ss-ffff}.db.gz", MainProcess.CurrentTime));
            }
        }


        public static async Task 加载数据()
        {
            ...
            {
                var buffer = await File.ReadAllBytesAsync(GameDataGateway.UserPath);
        ...
        public static void SortDataCommand(bool 保存数据)
        {
            ...
            foreach (KeyValuePair<Type, DataTableBase> keyValuePair in GameDataGateway.Data型表)
            {
                ...
                SEnvir.AddCommandLog(string.Format("{0} Sorted, Quantity sorted: {1}", keyValuePair.Key.Name, num3));
            }
            SEnvir.AddCommandLog(string.Format("The customer data has been collated, and the totals collated: {0}", num));
            if (num > 0 && 保存数据)
            {
                SEnvir.AddCommandLog("It may take a while to re-save the collated customer data, please wait...");
                GameDataGateway.强制保存();
                GameDataGateway.CleanUp();
                SEnvir.AddCommandLog("Data has been saved to disk");
                SEnvir.AddSystemError("Customer data has been collated, application needs to be restarted");
                Environment.Exit(0);
            }
        }
        ...
        {
            SEnvir.AddCommandLog("Start Clean Characters data...");
            ...
            foreach (GameData GameData in GameDataGateway.CharacterDataTable.DataSheet.Values.ToList<GameData>())
            {
                ...
                if (CharacterData != null && (int)CharacterData.CurrentRank.V < MinLevel && !(CharacterData.OfflineDate.V > t))
                {
                    ...
                    {
                        SEnvir.AddCommandLog(string.Format("[{0}]({1}/{2}) In the leaderboard, skipped cleanup", CharacterData, CharacterData.CurrentRank, (int)(DateTime.Now - CharacterData.OfflineDate.V).TotalDays));
                    }
                    else if (CharacterData.NumberDollars > 0)
                    {
                        SEnvir.AddCommandLog(string.Format("[{0}]({1}/{2}) There are unspent treasures, cleanup has been skipped", CharacterData, CharacterData.CurrentRank, (int)(DateTime.Now - CharacterData.OfflineDate.V).TotalDays));
                    }
                    else
                    {
                        ...
                        {
                            SEnvir.AddCommandLog(string.Format("[{0}]({1}/{2}) t's the president of the guild, skipped cleanup", CharacterData, CharacterData.CurrentRank, (int)(DateTime.Now - CharacterData.OfflineDate.V).TotalDays));
                        }
                        else
                        {
                            SEnvir.AddCommandLog(string.Format("Start cleaning [{0}]({1}/{2})...", CharacterData, CharacterData.CurrentRank, (int)(DateTime.Now - CharacterData.OfflineDate.V).TotalDays));
            ...
            SEnvir.AddCommandLog(string.Format("Character Data has been cleaned up, and the total has been cleared: {0}", num));
            if (num > 0)
            {
                SEnvir.AddCommandLog("It may take a while to re-save the cleaned customer data, please wait...");
        ...
        {
            SEnvir.AddCommandLog("Start collating current customer data...");
            GameDataGateway.SortDataCommand(false);
            Dictionary<Type, DataTableBase> dictionary = GameDataGateway.Data型表;
            SEnvir.AddCommandLog("Start loading the specified customer data...");
            GameDataGateway.Data型表 = new Dictionary<Type, DataTableBase>();
            foreach (Type type in Assembly.GetExecutingAssembly().GetTypes())
            {
                if (type.IsSubclassOf(typeof(GameData)))
            ...
            SEnvir.AddCommandLog("Start collating specified customer data...");
            DataLinkTable.处理任务();
            GameDataGateway.SortDataCommand(false);
            ...
            GameDataGateway.强制保存();
            GameDataGateway.CleanUp();
            SEnvir.AddCommandLog("The consolidation of customer data is complete");
            SEnvir.AddSystemError("Customer data has been merged, application needs to be restarted");
            Environment.Exit(0);
        }
